...
**`Allergen` fields:** `label` · `subcategory` · `category` · `allergen_key` · `is_active` · `created_at` · `updated_at`
...
Default is CATEGORY_CONTACT (single-category catalog; CATEGORY_OTHER, CATEGORY_FOOD, CATEGORY_INHALANT no longer exist).
...
No apostrophes, hyphens, spaces, or special characters. Example: `sodium_lauryl_sulfate` ✅ · `sls` ❌
