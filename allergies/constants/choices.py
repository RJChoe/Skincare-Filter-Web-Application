...
def build_flat_allergen_label_map(category_to_allergens_map):
    ...
    # Iterate through the values of the category map (which are lists of
    # (key, label) tuples)
    for allergen_list in category_to_allergens_map.values():
        # Update the flat_map with the tuples from the list.
        # Since dict() can take a list of tuples, this is a clean way to
        # merge. Note: If keys are duplicated across categories, the last
        # one processed wins.
